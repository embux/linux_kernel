		    ...
		    struct mxc_dispdrv_setting *setting)
{
	...
	struct display_timings *dis_ts;
	int ret;
	int i,j=0;
	struct fb_videomode *pfb_vm = &fb_vm;
	...
	dis_ts = of_get_display_timings(dev->of_node);
	if (!dis_ts) {
		...
		/* if native_mode = 0, maybe no native_mode assigned in the device tree */

		if(1) dev_err(dev, "\tldb: dft_mode = %s\n",setting->dft_mode_str);

		if (setting->dft_mode_str) {
			const char *name = setting->dft_mode_str;
			unsigned int namelen = strlen(name);
			int res_specified = 0, bpp_specified = 0, refresh_specified = 0;
			unsigned int xres = 0, yres = 0, bpp = setting->default_bpp, refresh = 0;
			...
			diff = -1;
			best = -1;
			for (i = 0; i < dis_ts->num_timings; i++) {

				ret = videomode_from_timings(dis_ts, &vidmode, i);	//convert a display timing to video mode <include/video/videomode.h>
				...
				if ( res_specified && res_matches(pfb_vm, xres, yres) )	{
					j = i;
					pfb_vm->refresh = refresh;
				}
			}
		}
		...
		ret = videomode_from_timings(dis_ts, &vidmode, j);	//convert a display timing to video mode <include/video/videomode.h>
...
static int ldb_probe(struct platform_device *pdev)
{
	...
	char clkname[16];
	int ebx_vdd_en, ebx_blk_en, ebx_timing;
	...
	for_each_child_of_node(np, child) {
		...
		// embux additional code
		ebx_vdd_en=of_get_named_gpio(child, "ebx-lvds-vdd-en", 0);
		ebx_blk_en=of_get_named_gpio(child, "ebx-lvds-blk-en", 0);
		ebx_timing=of_property_read_u32(child, "ebx-lvds-start-timing", &ebx_timing);
		if (0 != ebx_vdd_en && 0 != ebx_blk_en && 0 <= ebx_timing) {
			devm_gpio_request(dev, ebx_vdd_en, "ebx-lvds-vdd-en");
			devm_gpio_request(dev, ebx_blk_en, "ebx-lvd-blk-en");
			gpio_direction_output(ebx_vdd_en, 0);
			gpio_direction_output(ebx_blk_en, 0);
			gpio_set_value(ebx_vdd_en, 1);
			udelay(ebx_timing);
			gpio_set_value(ebx_blk_en, 1);
		}
		////////////////////////
		switch (mapping) {
