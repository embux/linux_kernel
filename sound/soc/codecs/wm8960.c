...
SOC_DAPM_SINGLE("Boost Switch", WM8960_LINPATH, 3, 1, 1),
};
		...
		int stream, int lrclk)
{
	...
	//snd_soc_update_bits(codec, WM8960_CLOCK1, 0x7 << 3, 2 << 3);
...
config_clock:
	 ...
	 */
	if (iface2 & (1<<6)) {
		snd_soc_update_bits(codec, WM8960_CLOCK1, 0x7 << 3, 1 << 3);
	}
			    ...
			    struct snd_soc_dai *dai)
{
	...
	iface = snd_soc_read(codec, WM8960_IFACE1) & 0xfff3;
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		for (i = 0; i < ARRAY_SIZE(alc_rates); i++)
			if (alc_rates[i].rate == params_rate(params)) {
				...
				snd_soc_update_bits(codec, WM8960_POWER1, 0x3e, 0x3e);
			}
	}
	...
	snd_soc_update_bits(codec, WM8960_POWER2, 0x180, 0x180);
	snd_soc_update_bits(codec, WM8960_ADDCTL3, 0x8, 0x0);
...
static int wm8960_mute(struct snd_soc_dai *dai, int mute)
{
	...
	if (mute)
		snd_soc_update_bits(codec, WM8960_DACCTL1, 0x8, 0x0);
	else
				      ...
				      enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_PREPARE:
		snd_soc_update_bits(codec, WM8960_POWER1, 0x1C0, 0xC0);	
		break;		
	...
	case SND_SOC_BIAS_STANDBY:
		if (codec->dapm.bias_level == SND_SOC_BIAS_OFF) {
			...
			snd_soc_write(codec, WM8960_POWER1, 0xc0);
		...
		int div_id, int div)
{
	...
	case WM8960_SYSCLKDIV:
		...
		snd_soc_write(codec, WM8960_CLOCK1, reg | /*div*/0x04);
		break;
	case WM8960_DACDIV:
					...
					unsigned int freq, int dir)
{
	...
	case WM8960_SYSCLK_MCLK:
		snd_soc_update_bits(codec, WM8960_CLOCK1,
					0x1, WM8960_SYSCLK_PLL /*WM8960_SYSCLK_MCLK*/);
		break;
...
static int wm8960_probe(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, WM8960_LINVOL, 0x13f, 0x13f);
	snd_soc_update_bits(codec, WM8960_RINVOL, 0x13f, 0x13f);
	...
	snd_soc_update_bits(codec, WM8960_ADDCTL2, 1<<6, 0<<6);
	//regmap_write(wm8960->regmap, WM8960_POWER2, 0x1e0);
	//regmap_write(wm8960->regmap, WM8960_POWER1, 0x0c0);
	
	snd_soc_update_bits(codec, WM8960_POWER1, 0x002, 0x002);
...
static int wm8960_display_register(struct snd_soc_codec* codec, char *buf, size_t count, loff_t pos) {
	return 0;
}
...
static struct snd_soc_codec_driver soc_codec_dev_wm8960 = {
	...
	//.idle_bias_off = true,
	//.display_register = wm8960_display_register,
};
