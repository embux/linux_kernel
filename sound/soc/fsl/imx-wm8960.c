...
static struct snd_soc_jack imx_hp_set, imx_mic_set;
...
static struct snd_soc_jack_pin imx_mic_set_pins[] = {
	{
		.pin = "Main MIC",
		.mask = SND_JACK_MICROPHONE,
	},
};
static struct snd_soc_jack_gpio imx_mic_set_gpio = {
	.name = "mic detected",
...
static int mic_set_status_check(void)
{
	struct imx_priv *priv = &card_priv;
	struct platform_device *pdev = priv->pdev;
	char *envp[3], *buf;
	...
	if (!gpio_is_valid(priv->mic_set_gpio))
		return 0;

	mic_status = gpio_get_value(priv->mic_set_gpio) ? 1 : 0;
	buf = kmalloc(32, GFP_ATOMIC);
	if (!buf) {
		dev_err(&pdev->dev, "%s kmalloc failed\n", __func__);
		return -ENOMEM;
	...
	if (false /*mic_status != priv->mic_active_low*/) {
		snprintf(buf, 32, "STATE=%d", 2);
		//snd_soc_dapm_disable_pin(&priv->codec->dapm, "Ext Spk");
		snd_soc_dapm_disable_pin(&priv->codec->dapm, "Main MIC");
		ret = imx_mic_set_gpio.report;
...
static int imx_wm8960_gpio_init(struct snd_soc_card *card)
{
	...
	if (gpio_is_valid(priv->mic_set_gpio)) {
		imx_mic_set_gpio.gpio = priv->mic_set_gpio;
		imx_mic_set_gpio.jack_status_check = mic_set_status_check;
		
		ret = snd_soc_jack_new(codec, "Main MIC",
				SND_JACK_MICROPHONE, &imx_mic_set);
		if (ret)
			return ret;
		ret = snd_soc_jack_add_pins(&imx_mic_set,
			ARRAY_SIZE(imx_mic_set_pins), imx_mic_set_pins);
		if (ret)
			return ret;
		ret = snd_soc_jack_add_gpios(&imx_mic_set, 1,
						&imx_mic_set_gpio);
		if (ret)
			return ret;
	}
...
static int imx_wm8960_late_probe(struct snd_soc_card *card)
{
	...
	snd_soc_dai_set_sysclk(codec_dai, WM8960_SYSCLK_MCLK, data->clk_frequency, SND_SOC_CLOCK_IN);	
...
static int imx_wm8960_probe(struct platform_device *pdev)
{
	...
	imx_wm8960_dai[0].ops = &imx_hifi_ops;
	imx_wm8960_dai[0].dai_fmt = SND_SOC_DAIFMT_I2S | SND_SOC_DAIFMT_NB_NF |
			    SND_SOC_DAIFMT_CBM_CFM;
	...
	priv->mic_set_gpio = of_get_named_gpio_flags(pdev->dev.of_node, "mic-det-gpios", 0,
			(enum of_gpio_flags *)&priv->mic_active_low);
	if (IS_ERR(ERR_PTR(priv->hp_set_gpio)))
		goto fail;	

	priv->mic_active_low=1;	
